...
#' Read and clean single sheet *calendar year* births from ONS
#'
#' @param file_path A character.
#' @param skip A double / integer.
#' @param sheet A character.
#' @param pivot_cols A character vector.
#' @returns A cleaned data frame.
read_and_clean_ss_births_lad <- function(
  file_path,
  skip = 5,
  sheet = "Table 1",
  pivot_cols = c("year", "gss_code", "gss_name")
) {
  cols_to_rename <- c(
    year = "Year",
...
#' Read and clean *calendar year* births from ONS, where each year is split across multiple sheets
...
read_and_clean_ms_births_lad <- function(file_path) {
  ...
  merged_cleaned_births <- lapply(wsheets, function(sheet) {
    data <- read_and_clean_ss_births_lad(
      file_path = file_path,
      skip = 4,
      sheet = sheet,
